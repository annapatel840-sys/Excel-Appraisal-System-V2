...
const express = require("express");
const catalyst = require("zcatalyst-sdk-node");
const { authenticate, canSeeEmployee, scopeEmployees } = require("./auth");
...
/* ============================================================
   AUTHENTICATION (every request below this line needs a login)
   ============================================================ */
...
/* ============================================================
   WHO AM I  (frontend calls this right after login)
   ============================================================ */
...
async function getEmployees(req, res) {
  ...
  // Tech-Ed sees only employees where appraiser_tech_ed = logged-in user
  const allEmployees = scopeEmployees(
    req.user,
    await getAllEmployees(datastore),
  );
...
async function createEmployees(req, res) {
  if (!req.user.permissions.canImport) {
    return sendJson(res, 403, {
      success: false,
      message: "Your role is not allowed to import or create employees.",
    });
  }
...
async function updateEmployee(req, res) {
  ...
  /* ==========================================================
     AUTHORIZATION
     1) the employee must be inside the user's scope
     2) locked fields may be re-sent unchanged, never changed
     ========================================================== */

  if (!canSeeEmployee(req.user, existingEmployeeRow)) {
    return sendJson(res, 403, {
      success: false,
  ...
  const lockedFields = req.user.permissions.lockedFields || [];

  for (let i = 0; i < lockedFields.length; i += 1) {
    const field = lockedFields[i];

    if (!Object.prototype.hasOwnProperty.call(body, field)) {
      continue;
    }

    const incomingValue = String(body[field] === null ? "" : body[field])
      .trim()
      .toLowerCase();

    const currentValue = String(
      existingEmployeeRow[field] === null ||
        existingEmployeeRow[field] === undefined
        ? ""
        : existingEmployeeRow[field],
    )
      .trim()
      .toLowerCase();
    ...
    if (incomingValue !== currentValue) {
      return sendJson(res, 403, {
        success: false,
        message: 'Your role is not allowed to change "' + field + '".',
      });
    }
  }
